...
## Step 5: 🔍 Examine array[1] = 2 (color: blue)
...
## Step 6: 🔵 Value is 2 (blue): swap array[1] ↔ array[4], decrement high (mid stays)
...
**Action:** Swap with `high` boundary and decrement `high`
- Swap `array[1]` (value: 2) with `array[4]` (value: 0)
- Calculation: `high = 4 - 1 = 3`
- Note: `mid` stays at 1 (need to examine swapped element)

**Swap Details:**
```
Before: array[1] = 2, array[4] = 0
After:  array[1] = 0, array[4] = 2
```

**Reasoning:**
- The 2 at position 1 belongs in the 2s region
- Swap it with the element at `high` boundary (position 4)
- Decrement `high` to expand the 2s region: 4 → 3
- **Don't advance `mid`** because the element swapped from `high` (value: 0) hasn't been examined yet
- Next iteration will examine this newly swapped element at position 1
...
## Step 7: 🔍 Examine array[1] = 0 (color: red)

**Current State:**
- Examining element at index `mid = 1`
- Value at mid: `0` (color: red)
- Pointer positions: `low = 1`, `mid = 1`, `high = 3`
...
- 0s (red): indices [0, 1) - **1 elements**
- 1s (white): indices [1, 1) - **0 elements**
- Unsorted: indices [1, 4) - **3 elements**
...
## Step 8: 🔴 Value is 0 (red): swap array[1] ↔ array[1], advance low and mid
