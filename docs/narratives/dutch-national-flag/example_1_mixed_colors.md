...
## Step 1: 🔍 Examine array[0] = 2 (color: blue)
...
## Step 2: 🔵 Value is 2 (blue): swap array[0] ↔ array[5], decrement high (mid stays)
...
- Swap `array[0]` (value: 2) with `array[5]` (value: 0)
- Calculation: `high = 5 - 1 = 4`
- Note: `mid` stays at 0 (need to examine swapped element)

**Swap Details:**
```
Before: array[0] = 2, array[5] = 0
After:  array[0] = 0, array[5] = 2
```

**Reasoning:**
- The 2 at position 0 belongs in the 2s region
- Swap it with the element at `high` boundary (position 5)
- Decrement `high` to expand the 2s region: 5 → 4
- **Don't advance `mid`** because the element swapped from `high` (value: 0) hasn't been examined yet
- Next iteration will examine this newly swapped element at position 0

**Updated Array:**
```
Index:   0   1   2   3   4   5
Value:   0   0   2   1   1   2
Color: red red blu whi whi blu
       LM          H     
```

---

## Step 3: 🔍 Examine array[0] = 0 (color: red)

**Current State:**
- Examining element at index `mid = 0`
- Value at mid: `0` (color: red)
- Pointer positions: `low = 0`, `mid = 0`, `high = 4`
...
- 0s (red): indices [0, 0) - **0 elements**
- 1s (white): indices [0, 0) - **0 elements**
- Unsorted: indices [0, 5) - **5 elements**
...
## Step 4: 🔴 Value is 0 (red): swap array[0] ↔ array[0], advance low and mid
...
## Step 5: 🔍 Examine array[1] = 0 (color: red)

**Current State:**
- Examining element at index `mid = 1`
- Value at mid: `0` (color: red)
- Pointer positions: `low = 1`, `mid = 1`, `high = 4`
...
- Unsorted: indices [1, 5) - **4 elements**
- 2s (blue): indices (4, 6) - **1 elements**
...
## Step 6: 🔴 Value is 0 (red): swap array[1] ↔ array[1], advance low and mid
...
## Step 7: 🔍 Examine array[2] = 2 (color: blue)

**Current State:**
- Examining element at index `mid = 2`
- Value at mid: `2` (color: blue)
- Pointer positions: `low = 2`, `mid = 2`, `high = 4`
...
- Unsorted: indices [2, 5) - **3 elements**
- 2s (blue): indices (4, 6) - **1 elements**
...
## Step 8: 🔵 Value is 2 (blue): swap array[2] ↔ array[4], decrement high (mid stays)
...
**Action:** Swap with `high` boundary and decrement `high`
- Swap `array[2]` (value: 2) with `array[4]` (value: 1)
- Calculation: `high = 4 - 1 = 3`
- Note: `mid` stays at 2 (need to examine swapped element)

**Swap Details:**
```
Before: array[2] = 2, array[4] = 1
After:  array[2] = 1, array[4] = 2
```

**Reasoning:**
- The 2 at position 2 belongs in the 2s region
- Swap it with the element at `high` boundary (position 4)
- Decrement `high` to expand the 2s region: 4 → 3
- **Don't advance `mid`** because the element swapped from `high` (value: 1) hasn't been examined yet
- Next iteration will examine this newly swapped element at position 2
...
## Step 9: 🔍 Examine array[2] = 1 (color: white)

**Current State:**
- Examining element at index `mid = 2`
- Value at mid: `1` (color: white)
- Pointer positions: `low = 2`, `mid = 2`, `high = 3`
...
- 0s (red): indices [0, 2) - **2 elements**
- 1s (white): indices [2, 2) - **0 elements**
- Unsorted: indices [2, 4) - **2 elements**
- 2s (blue): indices (3, 6) - **2 elements**

**Decision Point:** What to do with value `1`?

---

## Step 10: ⚪ Value is 1 (white): already in correct region, advance mid
...
## Step 11: 🔍 Examine array[3] = 1 (color: white)

**Current State:**
- Examining element at index `mid = 3`
- Value at mid: `1` (color: white)
- Pointer positions: `low = 2`, `mid = 3`, `high = 3`
...
- Unsorted: indices [3, 4) - **1 elements**
- 2s (blue): indices (3, 6) - **2 elements**
...
## Step 12: ⚪ Value is 1 (white): already in correct region, advance mid
